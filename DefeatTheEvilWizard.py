...
class Character:
    ...
    def attack(self, opponent, set_fixed_damage=0):
        ...
        if set_fixed_damage != 0:
            ...
            print(f"\n{self.name} attacks {opponent.name} for {set_fixed_damage} damage!")
        else:
            ...
            print(f"\n{self.name} attacks {opponent.name} for {self.damage_dealt} damage!")
    ...
    def heal(self, amount):
        ...
        print(f"\n{self.name} healed for {amount} health!")

    # Use the class StatusEffect to add a status to an entity.
    def add_status_effect(self, status_effect):
        # Prevents status effects from stacking. Instead, will just refresh tick duration.
        if status_effect.name in self.active_effect_names:
...
# Warrior Abilities.
    def rage(self):
        print(f'\n{self.name} used the ability "Rage"!')
...
class Mage(Character):
    ...
    def showAbilities(self, opponent):
        ...
        while self.choosing:
            ...
            self.choice = input("Choice: ").strip()
            if self.choice == "1":
                if "Fireball on cooldown" in self.active_effect_names:
                    for effect in self.status_effects:
                        if effect.name == "Fireball on cooldown":
...
class Druid(Character):
    ...
    def entangle(self, opponent):
        print(f'\n{self.name} used the ability "Entangle"!')
        opponent.add_status_effect(StatusEffect("Entangled", "dot", 3, 10))
        
    def rejuvenate(self):
        print(f'\n{self.name} used the ability "Rejuvenate"!')
...
class Paladin(Character):
    ...
    def law_of_equality(self, opponent):
        print(f'\n{self.name} used the ability "Law of Equality"!')
    ...
    def hammer_of_justice(self, opponent):
        print(f'\n{self.name} used the ability "Hammer of Justice"!')
...
class EvilWizard(Character):
    ...
    def regenerate(self, healAmount=5):
        print(f"\n{self.name} used Regenerate!")
        self.heal(healAmount)
        print(f"\n{self.name}'s Current health: {self.health}")
...
def battle(player, wizard):
    while wizard.health > 0 and player.health > 0:
        ...
        while choosing:
            print("\n--- Your Turn ---")
            print(f"1. Attack - Deal between {player.get_attack_power() - 10} - {player.get_attack_power()} damage")
        ...
        if player.health <= 0:
            ...
            print(f"\nOh no! {wizard.name} has defeated you!")
            break
